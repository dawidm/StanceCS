...
from utils import  spacy_seed_concepts_list,get_stance_dataset
...
def sentence_features(model, split, all_seeds, concept_graphs, relation_map, unique_nodes_mapping, max_words=5000, no_below=2):
    ...
    for j in tqdm(range(len(x)), position=0, leave=False):
        ...
        sg = generate_graph(xg, len(relation_map)).to(torch.device('cpu'))
        ...
        # if (j+1) %2000 == 0:
        #     np.ndarray.dump(sent_features, open('graph_features/sf_train'  + '_5000.np', 'wb'))
...
if __name__ == '__main__':
    ...
    # a = (train_triplets[:,1])
    # for i in range(38):
    #     print(str(i)+' '+str(sum(a==i)))
    n_bases = 4
    model = RGCN(len(unique_nodes_mapping), len(relation_map), num_bases=n_bases, dropout=dropout)
    model.load_state_dict(torch.load('weights/model_epoch' + str(300) +'.pt'))
    model.to('cpu')
    model.eval()
